...
export async function handleCouncil(
  ...
  if (request.method === 'POST' && url.pathname === '/api/v1/council/kick') {
    const body = await request.json() as { userId: string };

    const callerMembership = await env.DB.prepare(
      'SELECT council_id, role FROM council_members WHERE user_id = ?'
    ).bind(userId).first<{ council_id: string; role: string }>();

    if (!callerMembership || callerMembership.role !== 'steward') {
      return jsonResponse({ error: 'Only stewards can kick members' }, 403, corsHeaders);
    }

    if (body.userId === userId) {
      return jsonResponse({ error: 'Cannot kick yourself' }, 400, corsHeaders);
    }

    const targetMembership = await env.DB.prepare(
      'SELECT * FROM council_members WHERE user_id = ? AND council_id = ?'
    ).bind(body.userId, callerMembership.council_id).first();

    if (!targetMembership) {
      return jsonResponse({ error: 'User not in your council' }, 404, corsHeaders);
    }

    await env.DB.prepare(
      'DELETE FROM council_members WHERE council_id = ? AND user_id = ?'
    ).bind(callerMembership.council_id, body.userId).run();

    await syncGuildMembership(env.DB, body.userId, undefined);

    return jsonResponse({ success: true }, 200, corsHeaders);
  }

  if (request.method === 'POST' && url.pathname === '/api/v1/council/promote') {
    ...
    if (!callerMembership || callerMembership.role !== 'steward') {
      return jsonResponse({ error: 'Only stewards can promote members' }, 403, corsHeaders);
    }
    ...
    // Demote current steward
    await env.DB.prepare(
      'UPDATE council_members SET role = ? WHERE user_id = ? AND council_id = ?'
    ).bind('member', userId, callerMembership.council_id).run();

    // Promote new steward
    await env.DB.prepare(
      'UPDATE council_members SET role = ? WHERE user_id = ? AND council_id = ?'
    ).bind('steward', body.userId, callerMembership.council_id).run();

    // Update council table
    await env.DB.prepare(
      'UPDATE councils SET steward_user_id = ? WHERE id = ?'
    ).bind(body.userId, callerMembership.council_id).run();
